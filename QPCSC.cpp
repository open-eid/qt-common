...
{
	QStringList readers = QString::fromLocal8Bit(rawReaders()).split(QChar(0));
	readers.removeAll({});
	return readers;
}
...
void QPCSC::run()
{
	...
	while(!isInterruptionRequested())
	{
		...
		if(SC(GetStatusChange, d->context, 5*1000, list.data(), DWORD(list.size())) != SCARD_S_SUCCESS)
			continue;
